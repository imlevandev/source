...
struct FTransform
{
	...
	Matrix4x4 ToMatrixWithScale()
	{
		...
		float x2 = static_cast<float>(rot.x + rot.x);
		float y2 = static_cast<float>(rot.y + rot.y);
		float z2 = static_cast<float>(rot.z + rot.z);

		float xx2 = static_cast<float>(rot.x * x2);
		float yy2 = static_cast<float>(rot.y * y2);
		float zz2 = static_cast<float>(rot.z * z2);
		m._11 = (1.0f - (yy2 + zz2)) * static_cast<float>(scale.x);
		m._22 = (1.0f - (xx2 + zz2)) * static_cast<float>(scale.y);
		m._33 = (1.0f - (xx2 + yy2)) * static_cast<float>(scale.z);

		float yz2 = static_cast<float>(rot.y * z2);
		float wx2 = static_cast<float>(rot.w * x2);
		m._32 = (yz2 - wx2) * static_cast<float>(scale.z);
		m._23 = (yz2 + wx2) * static_cast<float>(scale.y);

		float xy2 = static_cast<float>(rot.x * y2);
		float wz2 = static_cast<float>(rot.w * z2);
		m._21 = (xy2 - wz2) * static_cast<float>(scale.y);
		m._12 = (xy2 + wz2) * static_cast<float>(scale.x);

		float xz2 = static_cast<float>(rot.x * z2);
		float wy2 = static_cast<float>(rot.w * y2);
		m._31 = (xz2 + wy2) * static_cast<float>(scale.z);
		m._13 = (xz2 - wy2) * static_cast<float>(scale.x);
...
static Matrix4x4 CreateMatrix(Vector3 rot, Vector3 origin) {
	const float DEG_TO_RAD = static_cast<float>(3.14159265358979323846) / 180.f;
	const float radPitch = static_cast<float>(rot.x) * DEG_TO_RAD;
	const float radYaw = static_cast<float>(rot.y) * DEG_TO_RAD;
	const float radRoll = static_cast<float>(rot.z) * DEG_TO_RAD;
...
static Vector3 WorldToScreen(FMinimalViewInfo camera, Vector3 WorldLocation) 
{
	...
	const Matrix4x4 tempMatrix = CreateMatrix(camera.Rotation, Vector3(0, 0, 0));
