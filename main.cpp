...
int main()
{
    if (!DRV::Init())
    {
        std::cout << "[!] Vui lòng nạp driver trước khi chạy ứng dụng!" << std::endl;
        Sleep(3000);
        return 0;
    }

    std::cout << "[+] Dang cho DeadByDaylight-Win64-Shipping.exe..." << std::endl;
    ...
    ScreenCenterX = static_cast<float>(GetSystemMetrics(SM_CXSCREEN)) / 2.0f;
    ScreenCenterY = static_cast<float>(GetSystemMetrics(SM_CYSCREEN)) / 2.0f;
    g_overlay = { 0 };
    _init(g_overlay);
}
