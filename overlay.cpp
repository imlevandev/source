...
auto FOverlay::draw_line(float x1, float y1, float x2, float y2, D2D1::ColorF color) -> void {
...
auto FOverlay::draw_boxnew(float x, float y, float width, float height, D2D1::ColorF color) -> void {
...
auto FOverlay::draw_text(float x, float y, D2D1::ColorF color, const char* str, ...) -> void {
	char buf[4096];
	wchar_t b[256];
	...
	const size_t len = min(strlen(buf), _countof(b) - 1);
	mbstowcs(b, buf, len);
	b[len] = L'\0';
	ID2D1SolidColorBrush* temp;
	...
	tar->DrawText(b, static_cast<UINT32>(len), format, D2D1::RectF(x, y, 1920.0f, 1080.0f), temp,
